function initializejQueryFunctions(memberId) {
                ...
                fetch(`/api/orders/cancel/${orderId}`, {
                    ...
                    .then(result => {
                        ...
                        if (result.message === "訂單已取消，並回收獲得點數和退還使用的點數。") {
                            ...
                            // 找到訂單狀態的單元格 (第5欄)
                            const $statusCell = $(`button[data-order-id="${orderId}"]`,).closest('tr').find('td:nth-child(5)');

                            // 更新 UI - 清空原有內容
                            $statusCell.empty();

                            // 創建一個新的 span 元素，並賦予 "status-cancelled" 類
                            const span = $('<span>', {
    ...
    var storageSite = "https://localhost:44354/";
    // 显示订单详细信息的函数
    function showOrderDetails(orderId) {
    ...
    ${order.items.map((item) => {
        ...
        var quantity = item.quantity || 1;
        var imageURL = item.image ? `${storageSite}uploads/Products/${item.image}` : '/path/to/default/image.jpg';
