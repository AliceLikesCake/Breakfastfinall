        ...
        public ActionResult UpdateOrderStatus(OrderDetailVm model)
        {
            try
            {
                ...
                if (order != null)
                {
                    ...
                    // 查找對應的會員
                    var member = _db.Members.FirstOrDefault(m => m.Id == order.MemberID); // 使用會員 ID 關聯

                    if (pointDetail != null && member != null)
                    {
                        // 如果訂單狀態是棄單 (假設4代表棄單)
                        if (model.OrderStatus == 4)
                        {
                            // 更新會員的點數 (棄單情況)
                            member.Points = member.Points + pointDetail.Used - pointDetail.Earned;

                            // 將點數的使用和獲得歸零
                            pointDetail.Used = 0;
                            pointDetail.Earned = 0;
                            pointDetail.Remaining = pointDetail.Original;
                        }
                    ...
                    {
                        return Json(new { success = false, message = "未找到對應的點數記錄或會員" });
                    }
                }
                else
                {
